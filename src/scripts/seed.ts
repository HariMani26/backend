...
const TN_DISTRICTS: Array<{ name: string; nameTa: string }> = [
...
const KULAMS: Array<{ valueEn: string; labelEn: string; labelTa: string }> = [
  ...
  { valueEn: 'ponnanipullu', labelEn: 'Ponnanipullu', labelTa: 'பொன்னானிப்புள்ளு' },
  ...
  { valueEn: 'puruvalapullu', labelEn: 'Puruvalapullu', labelTa: 'புருவலபுள்ளு' },
  { valueEn: 'manchalu', labelEn: 'Manchalu', labelTa: 'மஞ்சலு' },
  { valueEn: 'kachaipullu', labelEn: 'Kachaipullu', labelTa: 'கச்சால்புள்ளு' },
  { valueEn: 'koththiparanki', labelEn: 'Koththiparanki', labelTa: 'கொத்திபரங்கி' },
  { valueEn: 'seelajanaru', labelEn: 'Seelajanaru', labelTa: 'சீலஜானாரு' },
  { valueEn: 'ilakkimanu', labelEn: 'Ilakkimanu', labelTa: 'இளக்கிமானு' },
];

/**
 * PLACEHOLDER compatibility matrix — deterministic rotation over the 12 kulams so every
 * kulam gets a full, non-degenerate excellent(3)/good(3)/average(2)/avoid(self+3) bucket
 * set. This is NOT real community data. It must be replaced with authentic pairings
 * (confirmed by the product owner / community elders) before this goes live — see the
 * Phase 2 plan's "Key decision" and Open Questions sections.
 */
function buildPlaceholderCompatibilityMatrix(): Array<{
  kulam: string;
  excellent: string[];
  good: string[];
  average: string[];
...
async function seedKulams(): Promise<void> {
  for (const kulam of KULAMS) {
    await KulamModel.updateOne({ valueEn: kulam.valueEn }, { $set: kulam }, { upsert: true });
  }
  console.log(`✅ Seeded ${KULAMS.length} kulams`);
}

async function seedKulamCompatibility(): Promise<void> {
  const matrix = buildPlaceholderCompatibilityMatrix();
  for (const entry of matrix) {
    await KulamCompatibilityModel.updateOne({ kulam: entry.kulam }, { $set: entry }, { upsert: true });
  }
  console.log(`⚠️  Seeded ${matrix.length} PLACEHOLDER kulam-compatibility rows — replace with real data before go-live`);
}
  ...
  await MembershipPlanModel.updateOne(
    { name: 'Standard' },
    {
      $set: {
        ...
        features: ['Full search access', 'View photos & contact details', 'Send unlimited interests', 'WhatsApp chat after mutual interest'],
        isActive: true,
      },
    },
    { upsert: true },
