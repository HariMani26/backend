...
function requireFile(req: Request): IncomingFile {
  ...
  return { buffer: req.file.buffer, mimetype: req.file.mimetype, originalname: req.file.originalname, size: req.file.size };
}

export const uploadController = {
  uploadProfileImage: asyncHandler(async (req: Request, res: Response) => {
    const file = requireFile(req);
    const result = await uploadService.uploadPhoto(req.user!.id, 'profile-image', file);
    sendSuccess(res, result, 'Profile image uploaded', 201);
  }),

  uploadGalleryImage: asyncHandler(async (req: Request, res: Response) => {
    const file = requireFile(req);
    const result = await uploadService.uploadPhoto(req.user!.id, 'gallery', file);
  ...
  replaceFile: asyncHandler(async (req: Request, res: Response) => {
    const file = requireFile(req);
    const result = await uploadService.replaceFile(req.user!.id, req.params.id, file);
    sendSuccess(res, result, 'File replaced');
  }),

  deleteFile: asyncHandler(async (req: Request, res: Response) => {
    await uploadService.deleteFile(req.user!.id, req.params.id);
