...
export const profileController = {
  me: asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!profile) {
      throw ApiError.notFound('No profile found for this account yet — complete registration first');
    }
  ...
  getById: asyncHandler(async (req: Request, res: Response) => {
    ...
    const photoDocs = await uploadedFileRepository.findByUser(profile.userId);
    const photoFiles = photoDocs.filter((file) => (PHOTO_TYPES as readonly string[]).includes(file.type));
    const photoUrls = await Promise.all(
      photoFiles.map((file) => blobStorageService.generateReadSasUrl(file.container as BlobContainerKey, file.blobName)),
    );

    sendSuccess(res, toProfileDetail(profile, { hasFullAccess, photoUrls }), 'Profile detail');
  }),
};
